...
function initializeDb() {
    ...
    // Verificar si la tabla de PCs está vacía para poblarla.
    const countStmt = db.prepare('SELECT count(*) as count FROM pcs');
    ...
    if (count === 0) {
        ...
        const insertLog = db.prepare('INSERT INTO logs (pcId, pcName, timestamp, action, status, message, versionId) VALUES (@pcId, @pcName, @timestamp, @action, @status, @message, @versionId)');

        const populateTransaction = db.transaction(() => {
            for (const item of pcs) insertPc.run(item);
            for (const item of logs) insertLog.run(item);
        });
