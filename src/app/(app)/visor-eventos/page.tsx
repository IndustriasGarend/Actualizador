...
import { getLogs, clearLogs } from "@/lib/system-logger";
import type { SystemLogEntry, DateRange } from "@/lib/types";
import { RefreshCw, Trash2, Calendar as CalendarIcon, FilterX, Download, Loader2, ShieldAlert } from "lucide-react";
...
export default function LogViewerPage() {
  ...
  }, [logTypeFilter, debouncedSearchTerm, dateFilter]);

  const handleClearLogs = async () => {
    // Usamos un placeholder para el nombre de usuario
    await clearLogs("usuario_admin", logTypeToDelete, deleteAllTime);
  ...
  const getBadgeVariant = (type: SystemLogEntry['type']) => {
    switch (type) {
  ...
  return (
      <main className="flex flex-col h-full">
         <PageHeader
            ...
            description="Eventos, advertencias y errores registrados en la aplicación."
        />
        <div className="flex-1 p-6">
            ...
            <CardHeader>
                ...
                <div>
                    <CardTitle className="flex items-center gap-2"><ShieldAlert /> Registros del Sistema</CardTitle>
                    <CardDescription>
                    Herramienta de diagnóstico para monitorear la salud de la aplicación.
                    </CardDescription>
                </div>
